...
> **文档定位**：本文档是知识库（KB）构建的完整规格参考，为工程师提供可直接落地的条目示例、字段说明和构建规程。Metric KB 条目规格（§1）和 FPL Schema（§2）为当前有效规格。Baseline Profile 相关内容（§3）在当前设计中已不使用，可跳过。概要说明见 `design/v5.md Part V`。
