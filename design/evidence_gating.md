...
> **文档定位**：本文档是 GATE 节点详细实现规格，是工程师实现 `agent/gate.py` 的直接依据。概要原理见 `design/v5.md Part VI`。
>
> **核心设计问题**：单假设视角的收敛规则无法处理多假设并行的协调问题——如何判断多个假设能否同时通过，以及如何区分真正的复合故障与继发症状链。本文档提供 Rule A/B/C/D 详细规格、级联检测算法和完整决策伪代码。
