...
class Evaluator:
    ...
    def __init__(self, use_label_mapper: bool = False):
        """Initialize evaluator.

        Args:
            use_label_mapper: If True, use LLM-based LabelMapper to convert
                            natural-language fault descriptions to canonical
                            labels before evaluation. This is independent from
                            the agent pipeline.
        """
        self._label_mapper = LabelMapper() if use_label_mapper else None
    ...
    def evaluate_single(
        ...
        Args:
            mapped_labels: If provided, use these canonical labels instead of
                          report.root_causes[i].fault_type for matching.
                          Generated by LabelMapper (independent from agent).

        Returns metrics dict with hit_at_1, hit_at_3, composite_coverage,
        false_positives, tool_calls, latency_seconds, etc.
        """
        predicted_types = mapped_labels if mapped_labels else [rc.fault_type for rc in report.root_causes]
    ...
    def evaluate_batch(
        ...
        for exp_dir in sorted(results_path.iterdir()):
            ...
            # Use LabelMapper to map natural-language fault descriptions → canonical labels
            mapped = None
            if self._label_mapper:
                try:
                    mapped = self._label_mapper.map_report(report)
                    logger.info("Label mapping for %s: %s → %s",
                               exp_dir.name,
                               [rc.fault_type for rc in report.root_causes],
                               mapped)
                except Exception as e:
                    logger.warning("Label mapping failed for %s: %s", exp_dir.name, e)

            metrics = self.evaluate_single(report, gt, mapped_labels=mapped)
